...
namespace Bank_app.Forms
{
    public partial class CommunalPayments : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            var phoneNumber = textBoxPersonalAccount.Text;
            ...
            if (error == false)
            {
                ...
                if (DataStorage.attemps > 0)
                {
                    ...
                    var queryTransaction2 = $"insert into trandactions(transaction_type, transaction_destination, transaction_date, transaction_number, transaction_value, id_bank_card) values ('Оплата коммунальных услуг', '{comboBoxCommunalPayment.GetItemText(comboBoxCommunalPayment.SelectedItem)}','{transactionDate}','{transactionNumber}', '{sum}', (select id_bank_card from bank_card where bank_card_number = '{cardNumber}'))";
                    ...
                    var command2 = new SqlCommand(queryTransaction1, dataBase.getConnection());
                    var command3 = new SqlCommand(queryTransaction1, dataBase.getConnection());
                    var command4 = new SqlCommand(queryTransaction1, dataBase.getConnection());
